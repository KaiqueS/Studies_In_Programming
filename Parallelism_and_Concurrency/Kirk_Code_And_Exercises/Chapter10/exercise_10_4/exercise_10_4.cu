...
__global__ void CoarsenedSumReductionKernel( int* input, int* output, unsigned int Coarse_Factor ){
	...
	__shared__ extern float shared_input[ ]; // Shared-Memory size := block dimension BY CONSTRUCTION at kernel call through execution configuration parameters
	...
	for( unsigned int tile = 1; tile < ( Coarse_Factor * 2 ); ++tile ){
		...
		greatest = ( input[ i ] > input[ ( tile * blockDim.x ) + i ] ) ? input[ i ] : input[ ( tile * blockDim.x ) + i ];
	}
...
void kernel_setup( int*& host_array, int& output, int size ){
	...
	cudaMalloc( ( void** ) &dev_out, sizeof( int ) );

	cudaMemcpy( dev_array, host_array, array_size, cudaMemcpyHostToDevice );
	cudaMemcpy( dev_out, &output, sizeof( int ), cudaMemcpyHostToDevice );
	...
	unsigned int num_threads{ static_cast<unsigned int>( size ) / coarse_factor };

	dim3 threads{ num_threads, 1, 1 };

	int shared_memsize = size * sizeof( int );
